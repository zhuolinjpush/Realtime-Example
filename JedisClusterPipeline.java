...
//    public static JedisClusterPipeline pipelined(JedisCluster cluster) {
...
//    }
    ...
    public static JedisClusterPipeline pipelined(String hosts) {
        JedisClusterPipeline pipeline = null;
        try {
            nodes = new HashSet<HostAndPort>();
            for (String host : hosts.split(",")) {
                String[] arr = host.split(":");
                logger.info("host:port="+arr[0]+":"+Integer.parseInt(arr[1]));
            ...
            logger.error("init jedis error:"+e);
        }
    ...
    public static void main(String[] args) throws IOException {
        String redisNodes = SystemConfig.getProperty("nanji.redis.hosts");

        long s = System.currentTimeMillis();
        JedisClusterPipeline jcp;

        jcp = JedisClusterPipeline.pipelined(redisNodes);
        jcp.refreshCluster();
        List<Object> batchResult = null;
        try {
            // batch write
            for (int i = 0; i < 10000; i++) {
                jcp.incrBy("test_wenqh", 1);
                jcp.expire("test_wenqh", 3600);
            }
            jcp.sync();
